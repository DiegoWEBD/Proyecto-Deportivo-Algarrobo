...
function Inicio() {
    ...
    //var moment_dia = moment(year_global + '-0' + mes_global + '-0' + dia_global + ' ' + hora_global + ':00:00');
...
function CargarNombreProfesor(id_calendario) {
    $.ajax({
        type: 'POST',
        url: 'BuscarNombreProfesor',
        dataType: 'json',
        data: { idCalendario: id_calendario },
        beforeSend: function () {
           //MostrarLoader('loader', 'div_contenido')
        },
        success: function (response) {
            ...
            if (correcto == true) {
                if (mensaje.length > 0) {
                    $("#form_ReservarHora input[name = nombreProfesor]").val(mensaje[0][0]);
                }
                else {
                    $("#form_ReservarHora input[name = nombreProfesor]").val("Sin Asignación de Profesor");
                }
            }
            else {
                $('#Error_ReservarHora').empty();
...
function ChangeFecha() {
    ...
    $("#form_ReservarHora input[name = nombreProfesor]").val("");
    EstadoCupos(false, true, 0, 0);
}
...
function ChangeSelectHorario(entrada) {
    ...
    if (objeto.localeCompare("-1") != 0) {
        ...
        CargarNombreProfesor(idCalendario);
    }
    else {

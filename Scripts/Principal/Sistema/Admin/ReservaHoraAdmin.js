...
function EnviarModalUsuario() {
    //limpiar elementos
    $("#FormDatosUsuario input[name = nombreUsuario]").val("");
    ...
    $("#checkMonto")[0].checked = false;
    $("#MontoClase").prop('disabled', true);
    $("#MontoClase").prop('required', false);
    myModal.show();
}
...
function RealizarReserva() {
    ...
    if (objeto.localeCompare("-1") != 0) {
        ...
        var boolMonto = $("#checkMonto")[0].checked;
        var monto = $("#FormDatosUsuario input[name = monto]").val();
        EjecutarReserva(fecha_arreglo, idCalendario, rut, nombre, app, apm, correo, boolMonto, monto );
    }
    else {
...
function EjecutarReserva(arreglo_fecha, id_calendario, rut, nombre, app, apm, correo, boolMonto, monto ) {
    $.ajax({
        type: 'POST',
        url: 'CrearReservaDia',
        dataType: 'json',
        data: { dia: arreglo_fecha[0], mes: arreglo_fecha[1], year: arreglo_fecha[2], idCalendario: id_calendario, rut: rut, nombre: nombre, app: app, apm: apm, correo: correo, boolMonto: boolMonto, monto: monto },
        beforeSend: function () {
            MostrarLoader('loader', 'div_contenido')
        },
        success: function (response) {
            ...
            var myModalEl = document.getElementById('datosInvitadoModal');
...
function EstadoPago() {
    ...
    var objeto = $("#checkMonto")[0].checked;
    if (objeto) {
        $("#MontoClase").prop('disabled', false);
        $("#MontoClase").prop('required', true);
    }
    else {
        $("#MontoClase").prop('disabled', true);
        $("#MontoClase").prop('required', false);
...
function ComprobarHorario(entrada) {
    ...
    if (esHoy()) {
        ...
        //return false;
        return true; //para entregar todos lo horarios disponibles+
    }
    return true;
}
